...
pub fn encrypt_file(path: &String, password: &String, pg: &PrimeGenerator) -> io::Result<()> {
	...
	if prime_vector.len() < pw_vector.len(){
		for i in prime_vector.len()..pw_length {
			prime_vector.push(prime_vector[i % prime_vector.len()]);
		}	
	}
	else{	
	...
	// Algorithm assumes that file is at least 256 bytes long
	// TODO Put this earlier
	if file_contents_modified.len() < 256 {
	...
	for i in 0..wombo_combo.len(){
		let position = pw_vector[pw_vector.len()-1] as usize;
...
pub fn decrypt_file(path: &String, password: &String, _pg: &PrimeGenerator) -> io::Result<()> {
	...
	if pw_vector != old_password{
		return Err(Error::new(ErrorKind::Other, "Password mismatch"));
	}
